...
class APIServices{
  ...
  Future<void> addSS(SSList lista) async{
    try{
      ...
      for(SS x in lista.items!){
        var response = await client.post(uri,body: json.encode(x.toJson()),headers: headers);    
  ...
  Future<void> updateSS(int id, String tipo) async{
    try{
      ...
      print(map["items"]);
      var response= await client.put(uri,body:json.encode(x.toJson()),headers: headers);
      client.close();
...
static Future<void> addUtente(Utente u) async{
    try{
      ...
      var response= await client.post(uri,body:json.encode(u.toJson()),headers: headers);
      client.close();
...
static Future<Utente?> getUtente(String email, String password) async{
  try{
    var client= http.Client();
    var uri=Uri.parse("${RestAPIs.baseURL}${RestAPIs.utenteAccount}/$email/$password");
    var response = await client.get(uri);
    client.close();
    log(response.statusCode.toString());
    if(response.statusCode==200){
...
static Future<void> addRicetta(Ricetta r) async{
 try{
  ...
  var response = await client.post(uri,body: json.encode(r.toJson()),headers: headers);
  client.close();
  log(response.statusCode.toString());
...
static Future<void> addScansione(Scansione s) async{
  try{
    var client= http.Client();
    var uri= Uri.parse("${RestAPIs.baseURL}${RestAPIs.ricettaMedica}/scansione");
    var response = await client.post(uri,body: json.encode(s.toJson()),headers: headers);
    client.close();
    log(response.statusCode.toString());
    if(response.statusCode==200 || response.statusCode==201){
      print("POST FUNZIONA");
